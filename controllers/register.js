            ...
            if (isTorreIdValid) {
                ...
                db.transaction(trx => {
                    trx.insert({
                        ...
                        .catch(error => {
                            trx.rollback;
                            res.status(400).json('Torre Id already registered');
                        })
                });
            } else {
                res.status(400).json('Invalid Torre Id');
            }

        } else {
            res.status(400).json('Name must be > 2 chars, torreid > 2 chars and password > 5 chars');
        }
